    ...
    >
      ...
      >
        ...
        <el-form-item label="订单号:">
          <el-input v-model="storeInfo.orderNum" disabled> </el-input>
        </el-form-item>
        <el-form-item label="产品名称:">
          <el-input v-model="storeInfo.name" disabled> </el-input>
        </el-form-item>
        <el-form-item label="存入数量:">
          <el-input v-model="storeInfo.complete" disabled> </el-input>
        </el-form-item>
        <el-form-item label="生产线:">
          <el-input v-model="storeInfo.outputId" disabled> </el-input>
        </el-form-item>
        <!-- 远程搜索-->
        <el-form-item label="占地面积:">
          <el-input
            v-model="store.area"
            placeholder="货架水平占地面积为10平方米"
          >
          </el-input>
        </el-form-item>
        <p v-if="store.area > 0">
          提示:大约占用{{ Math.ceil(store.area / 10) }}层货架
        </p>
        <!--级联选择器-->
  ...
  methods: {
    ...
    async location(item) {
      ...
      const res = await this.$http.get(
        "/transition/transitionPosition/" + item.id
      );
      console.log(res.obj);
      if (res.obj.length === 0) {
        res.obj = "未存待检库或已入库";
      ...
      MessageBox.confirm("此操作将永久删除本条生产数据，是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await this.$http.delete(
            "/transition/deleteTransition/" + id
          );
          if (res) {
            this.getTransition();
          }
